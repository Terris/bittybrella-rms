...
const StockCategoryForm = (props) => {
  const onSubmit = (data) => {
    props.onSave(data, props?.stockCategory?.id)
  }
  ...
  return (
    <div className="box-border text-sm -mt-4">
      <Form onSubmit={onSubmit} error={props.error}>
        ...
        <TextField
          name="name"
          defaultValue={props.stockCategory?.name}
          className={CSS.input}
        ...
        <TextField
          name="stockItemId"
          defaultValue={props.stockCategory?.stockItemId}
          className={CSS.input}
          errorClassName={CSS.inputError}
          validation={{ required: true }}
        />
        <FieldError name="stockItemId" className={CSS.errorMessage} />
